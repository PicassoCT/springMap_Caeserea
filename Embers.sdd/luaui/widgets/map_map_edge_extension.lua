...
function widget:GetInfo()
  return {
    name      = "Map Edge Extension (map widget)",
    version   = "v0.5",
    desc      = "Draws a mirrored map next to the edges of the real map",
    author    = "Pako",
    date      = "2010.10.27 - 2011.10.29",
    license   = "GPL",
    layer     = 2,
    enabled   = true,
    --detailsDefault = 3
...
function widget:Initialize()
	...
	-- abort on metal factions because it already has map edge extension
	if (string.match(Game.gameShortName, "metal_factions")) then
		return
	end
	
	-- ZK workaround
	if  WG.game_SetCustomExtensionGridTexture then
		WG.game_SetCustomExtensionGridTexture(gridTex,true)
		return
	end
