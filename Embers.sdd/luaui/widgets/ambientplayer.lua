...
local versionNum = '0.11a'

function widget:GetInfo()
  return {
    name      = "Ambient Player",
    desc      = "v"..(versionNum).." modified ambient sound mixed for lavamaps",
    author    = "Klon, The_Yak",
    date      = "dez 2014",
...
local LAVABURN_FILE = 'LuaUI/sounds/ambient/lavaburn.lua'
...
function widget:Initialize()	
	if VFS.FileExists(LAVABURN_FILE, VFS.RAW_FIRST) then
		if (Spring.LoadSoundDef(LAVABURN_FILE)) then
			Spring.Echo("Loaded BURN Sound Defs successfully")
		end
	end
...
function widget:Update(dt) 	
	...
	local track = 1
	local params = tracks['lavaambient'..track];
